#include "../header/gpio.h"
#include "../header/i2c.h"
#include "../header/reg.h"
#include "../header/timer.h"

/*
 REFDIV:    1
 FBDIV:     125
 VCO:       1500.0 MHz
 PD1:       5
 PD2:       2
 */
void init() {
  XOSC_CTRL &= ~(0xFFF << 12);
  XOSC_CTRL |= (0xFAB << 12) | 0xAA0;

  while (!(XOSC_STATUS & (1 << 12))) {
  }
  while (!(XOSC_STATUS & (1 << 31))) {
  }
  CLK_REF_CTRL &= ~0x3;
  CLK_REF_CTRL |= 0x2;
  RESETS_RESET &= ~(1 << 14);
  while (!(RESETS_RESET_DONE & (1 << 14))) {
  }
  // PLL_SYS
  PLL_SYS_CS &= ~(0x3F << 0);
  PLL_SYS_CS |= 1; // REFDIV = 1

  PLL_SYS_FBDIV_INT = 125; // FBDIV
  PLL_SYS_PWR &= ~(1 << 0);
  PLL_SYS_PWR &= ~(1 << 5);

  while (!(PLL_SYS_CS & (1 << 31))) {
  }
  PLL_SYS_PRIM = (5 << 16) | (2 << 12);
  PLL_SYS_PWR &= ~(1 << 3);

  CLK_SYS_CTRL &= ~(7 << 5);
  CLK_SYS_CTRL |= (0 << 5);
  CLK_SYS_CTRL |= (1 << 0);
  CLK_PERI_CTRL &= ~(1 << 11);
  CLK_PERI_CTRL |= (1 << 11);
  CLK_PERI_CTRL &= ~(7 << 5);
  CLK_PERI_CTRL |= (0 << 5);

  TIMER0_SOURCE = 0;
  TICKS_TIMER0_CTRL &= ~1;
  TICKS_TIMER0_CYCLES = 12;
  TICKS_TIMER0_CTRL |= 1;
  while (!(TICKS_TIMER0_CTRL & (1 << 1))) {
  }
}
void enable_fpu() {
#if defined(ARCH_ARM)
  SCB_CPACR |= (0xF << 20); // enable CP10 and CP11
  __asm volatile("dsb");
  __asm volatile("isb");
#elif defined(ARCH_RISCV)

#else
#error "UNKNOWN ARCH"
#endif
}
void oled_test(rp2350_i2c_cfg_t *cfg) {
  /*
   * ============================================================
   * 1. SSD1306 INITIALIZATION
   * ============================================================
   */
  static const uint8_t init[] = {
      0x00, // Control: command stream

      0xAE, // Display OFF

      0xD5, 0x80, // Display clock divide / oscillator
      0xA8, 0x3F, // Multiplex ratio = 63 (128x64)
      0xD3, 0x00, // Display offset = 0
      0x40,       // Display start line = 0

      0x8D, 0x14, // Charge pump ON

      0x20, 0x00, // Horizontal addressing mode

      0xA1, // Segment remap
      0xC0, // COM scan direction NORMAL

      0xDA, 0x12, // COM pins hardware configuration

      0x81, 0x7F, // Contrast

      0xD9, 0xF1, // Pre-charge period
      0xDB, 0x40, // VCOMH deselect level

      0xA4, // Display follows RAM
      0xA6, // Normal (not inverted)

      0xAF // Display ON
  };

  i2c_master_write(cfg, init, sizeof(init));

  /*
   * ============================================================
   * 2. CLEAR DISPLAY
   *
   * 8 pages x 128 columns = 1024 bytes
   * ============================================================
   */

  static const uint8_t clear_cmd[] = {
      0x00,             // Command stream
      0x21, 0x00, 0x7F, // Columns 0..127
      0x22, 0x00, 0x07  // Pages 0..7
  };

  i2c_master_write(cfg, clear_cmd, sizeof(clear_cmd));

  uint8_t clear_data[129];

  clear_data[0] = 0x40; // Data stream

  for (int i = 1; i <= 128; i++) {
    clear_data[i] = 0x00;
  }

  for (int page = 0; page < 8; page++) {
    i2c_master_write(cfg, clear_data, sizeof(clear_data));
  }

  /*
   * ============================================================
   * 3. SELECT PAGE 3
   *
   * Page 3 contains physical rows 24..31.
   *
   * With 0xC0 COM direction:
   *
   *   bit 0 = row 31
   *   bit 1 = row 30
   *   ...
   *   bit 7 = row 24
   * ============================================================
   */

  static const uint8_t page3_cmd[] = {
      0x00, 0x22, 0x03, 0x03, // Page 3 only
      0x21, 0x00, 0x7F        // Columns 0..127
  };

  i2c_master_write(cfg, page3_cmd, sizeof(page3_cmd));

  /*
   * ============================================================
   * 4. DRAW ROW 31
   * ============================================================
   */

  uint8_t line31[129];

  line31[0] = 0x40; // Data

  for (int i = 1; i <= 128; i++) {
    line31[i] = 0x01; // bit 0 = row 31
  }

  i2c_master_write(cfg, line31, sizeof(line31));

  /*
   * ============================================================
   * 5. SELECT PAGE 4
   *
   * Page 4 contains physical rows 32..39.
   *
   * With 0xC0:
   *
   *   bit 0 = row 39
   *   bit 7 = row 32
   * ============================================================
   */

  static const uint8_t page4_cmd[] = {
      0x00, 0x22, 0x04, 0x04, // Page 4 only
      0x21, 0x00, 0x7F        // Columns 0..127
  };

  i2c_master_write(cfg, page4_cmd, sizeof(page4_cmd));

  /*
   * ============================================================
   * 6. DRAW ROW 32
   * ============================================================
   */

  uint8_t line32[129];

  line32[0] = 0x40; // Data

  for (int i = 1; i <= 128; i++) {
    line32[i] = 0x80; // bit 7 = row 32
  }

  i2c_master_write(cfg, line32, sizeof(line32));
}
void _start0(void) {

  enable_fpu();
  gpio_init(4, I2C);
  gpio_init(5, I2C);
  gpio_set(5, BOTH);
  gpio_set(4, BOTH);

  rp2350_i2c_cfg_t cfg = {.addr = 0x3C,
                          .scl_pin = 5,
                          .sda_pin = 4,
                          .speed = I2C_SPEED_FAST,
                          .operate = MASTER};
  i2c_init(&cfg);
  oled_test(&cfg);

  while (1) {
  }
  // RESETS_RESET &= ~((1u << 6) | (1U << 9));
  // while ((RESETS_RESET_DONE & ((1u << 6) | (1u << 9))) !=
  //        ((1u << 6) | (1u << 9))) {
  // }
  // GPIO25_CTRL &= ~(0x1F);
  // GPIO25_CTRL |= (0x05);
  // PADS_BANK0_GPIO25 |= (3 << 4);
  // PADS_BANK0_GPIO25 |= (1 << 2);
  // PADS_BANK0_GPIO25 &= ~(1 << 8);
  // SIO_GPIO_OE_SET = (1 << 25);
  // SIO_GPIO_OUT_XOR = (1 << 25);
  // while (1) {
  //   delay_s(2);
  //   SIO_GPIO_OUT_XOR = (1 << 25);
  // }
}
void _start1(void) {
  enable_fpu();
  // pwm_init();
  // pwm_set(25);
  // uint8_t percentage = 0;
  // int8_t direction = 1;
  gpio_init(25, SIO);
  gpio_set(25, OUTPUT);
  while (1) {
    gpio_out(25, HIGH);
    // pwm_duty(25, percentage);
    // percentage += direction;

    // if (percentage == 100) {
    //   direction = -1;
    // } else if (percentage == 0) {
    //   direction = 1;
    // }

    delay_ms(200);
    gpio_out(25, LOW);
    delay_ms(200);
  }
}
